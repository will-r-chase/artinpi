...
piChar <- read.table("data/PI_100000.txt", stringsAsFactors=F, colClasses = c("character"))[1,1]
...
pi_fancy <- Pi.frame %>%
  mutate(show_time = ifelse(ID %in% 1:100, 500, 1),
         reveal_time = cumsum(show_time)) %>%
  mutate(pause_time = ifelse(ID %in% c(1000, 5000, 7000), 500, 1)) %>%
  uncount(pause_time) %>%
  mutate(pause_sum = cumsum(pause_time))
